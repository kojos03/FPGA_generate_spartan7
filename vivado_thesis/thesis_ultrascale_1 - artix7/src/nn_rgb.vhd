...
architecture behave of nn_rgb is
    ...
    type   y_array is array (0 to 20) of std_logic_vector(7 downto 0);
    signal y : y_array;
...
control: entity work.control
    -- Increased to match the deeper NN pipeline (sigmoid LUT pipelined).
    generic map (delay => 19)
    port map (  clk      => clk,
                reset    => reset,
                vs_in    => vs_0,
                hs_in    => hs_0,
...
-- assign the pixel a value depending on the output of the neural network
      ...
      if(connection(11) > 127) then
            ...
            if(connection(11) > connection(10)) then
                -- yellow
                result_r <= '1' & y(18)(7 downto 1);
                result_g <= '1' & y(18)(7 downto 1);
                result_b <= '0' & y(18)(7 downto 1);
            else
                -- blue
                result_r <= '0' & y(18)(7 downto 1);
                result_g <= '0' & y(18)(7 downto 1);
                result_b <= '1' & y(18)(7 downto 1);
            end if;
